		...
		<el-header>
			<vxe-toolbar ref="ScheduleToolbar" custom print export>
				<template #buttons>
					...
					<vxe-button @click="print">打印</vxe-button>
				</template>
			</vxe-toolbar>
		</el-header>
		<el-main>
...
export default {
	...
	methods: {
		...
		page_list: function () {
			this.api.production.TaskPipe.GetStatList(this.search).then(res => {
						...
						if (!result[dataList[i].standard]) {
							result[dataList[i].standard] = {
								...
								childCols: [
									...
									{ field: "qty", title: "制管数量（支）", width: "120px", align: "right" },
									{ field: "totalLength", title: "制管总长度（米）", width: "140px", align: "right" },
									{ field: "totalWeight", title: "制管总重量（公斤）", width: "150px", align: "right" },
									{ field: "short_length", title: "裁管长度（米）", width: "120px", align: "right" },
									{ field: "pitch_num", title: "每支裁短管数量（支）", width: "170px", align: "right" },
