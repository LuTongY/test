<template>
   <div v-dialogdrag>
    <el-dialog v-model="is_show" width="700px" title="审核工序路线"  destroy-on-close top='7vh'>
          ...
          >
           ...
           <div style="height:40px;margin-top:8px">
             <el-button type="primary" @click="submitForm('roleForm')" style="float:right;margin:0 8px">通过</el-button>
             <el-button @click="AuditFail" style="float:right" >驳回</el-button>
           </div>
...
export default {
  ...
  methods: {
    ...
    submitForm(){
      this.api.production.ProcessRoute.AuditSuccess(this.select_id,this.materialCode,this.items).then((res)=>{
    ...
    AuditFail(){  
        this.api.production.ProcessRoute.AuditFail(this.select_id).then((res)=>{
            if(res.data.code==200){
                this.is_show=!this.is_show
