    ...
    <el-main>
      ...
      >
        ...
        <vxe-table-column
          field="status"
          title="状态"
          :formatter="({ cellValue })=>{ return  cellValue==1?'正常':'停用'}"
        ></vxe-table-column>
      </vxe-table>
      <template #empty>
        <span style="color: red;">
          <img src="@\components\img\err_table.gif" />
          <p>不用再看了，没有更多数据了！</p>
        </span>
      </template>
    </el-main>
...
export default {
  ...
  methods: {
    ...
    edit: function() {
      ...
      this.api.production.Workshop.EditInfo(this.select_id).then(res => {
    ...
    del: function() {
      ...
      this.$confirm("此操作将删除该工厂, 是否继续?", "提示", {
        ...
        this.api.production.Workshop.del(this.select_id)
          .then(res => {
            this.page_list();
