<template>
	<div v-dialogdrag>
		<el-dialog :title="(isReprint ? '补打' : '打印') + '流转卡'" v-model="PrintShow" destroy-on-close width="700px"
			top='5vh'>
			<el-container>
				<el-header class="el-dialog__header">
					<vxe-button @click="print(0)">打印</vxe-button>
					<span style="margin:0 10px;">计划总量：{{ table.totalQty }}</span>
					<span>当前可打数量：{{ table.totalQty - table.createQty }}</span>
				</el-header>
												...
												<tbody>
													<tr>
														...
														<td width="80" height="25"
															...
															<text v-if="isReprint">{{ table.iquantity }}</text>
															<input type="number" style="width: 60px;border: none;"
																v-model="table.iquantity" placeholder="子件数量"
																v-bind:readonly='isReprint' v-else />
														</td>
													</tr>
												</tbody>
											</table>
