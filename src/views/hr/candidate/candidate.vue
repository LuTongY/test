    ...
    <el-header>
      <vxe-toolbar ref="candidateToolbar" custom print export>
        <template #buttons>
          ...
          <vxe-button v-has="['in_blacklist']" @click="moveToBlacklist">移入黑名单</vxe-button>
...
export default {
  ...
  methods: {
    ...
    moveToBlacklist: function () {
      ...
      this.$confirm("确定要移入黑名单吗?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then(() => {
        this.api.hr.Candidate.MoveToBlacklist(ids).then((res) => {
          if (res.data.code == 200) {
