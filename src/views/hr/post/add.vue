<template>
  <div v-dialogdrag>
    <el-dialog title="增加职位" v-model="is_show" width="450px">
      <el-form :model="Form" :rules="rules" ref="Form" label-width="100px" class="demo-ruleForm">
        <el-form-item label="名称" prop="name">
          <vxe-textarea class="txtarea-item" v-model.trim="Form.name" resize="vertical" placeholder="支持添加多个职位，一行一个"
            style="width:220px;line-height: 1.8;"></vxe-textarea>
        </el-form-item>
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.api.hr.Post.add(this.Form).then(
            res => {
