...
<template>
  <div v-dialogdrag>
    <el-dialog title="权限" v-model="is_tree_show" width="500px">
	  <div style="padding-bottom:8px;text-align: right;">
	  	<el-switch
	  	    v-model="isMobile"
	  	    size="large"
	  	    active-text="app"
	  	    inactive-text="PC"
			@change="get_auth(select_id)"
	  	 />
	  </div> 
...
export default {
  ...
  methods: {
    get_auth: function(id) {
	  let type=this.isMobile?'GetMobileAuth':'GetAuth';
      this.api.user_list[type](id).then(res => {
    ...
    affirm_bt: function() {
	let type=this.isMobile?'UpdateMobileAuth':'UpdateAuth';	
      this.api.user_list[type](this.select_id, this.updateTreeId).then(res => {
        if (res.data.code == 200) {
