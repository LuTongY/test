	...
	<el-container>
			...
			<vxe-toolbar ref="WageToolbar" custom print export>
				<template #buttons>
					<el-row>
						...
						<title-date ref="TitleDate" @page_list="page_list" v-model:Pdata="search.date" days="M"  format="YYYY-MM" type='month' />
					</el-row>
				</template>
			</vxe-toolbar>
		</el-header>
		<el-main>
			<vxe-table :data="tableData" highlight-current-row highlight-hover-row height="auto" border="full"
				...
				<vxe-table-column field="staffNo" title="加工人工号" width="130px"></vxe-table-column>
				<vxe-table-column field="staffName" title="加工人姓名" width="150px"></vxe-table-column>
				<vxe-table-column field="totalPrice" title="工资" width="85px"></vxe-table-column>
	...
	export default {
		...
		methods: {
			...
			page_list: function() {
				...
				this.api.finance.Wage.GetList(pagesize, page, this.search).then(res => {
			...
			DownExcel:function(){
				this.api.finance.Wage.ExportStaffSummary(this.search).then(res=>{
					if(res.data.code==200){
