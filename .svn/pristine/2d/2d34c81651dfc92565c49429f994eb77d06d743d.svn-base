		...
		<el-dialog :title='title' v-model="is_show" :width="width" destroy-on-close>
			<el-container>
				<el-main>
					<el-row :gutter="20">
						<el-col :span="6">
							<vxe-table :data="data.staffList" highlight-current-row highlight-hover-row height="600"
								border="full" row-id="userwork" keep-source ref="user" @radio-change="UpdateUser"
								:auto-resize="$store.state.autoResize"
								:radio-config="{ labelField: 'name', trigger: 'row' }">
								<vxe-column type="radio" title="选择" align="center" width="60px"></vxe-column>
								<vxe-column field="staffName" title="人员" align="left">
									<template #default="{ row }">
										{{ row.staffName + '(' + row.staffNo + ')' }}
									</template>
								</vxe-column>
							</vxe-table>
						</el-col>
						<el-col :span="18">
							<vxe-table :data="data.inventoryList" ref='allocation' highlight-current-row
								highlight-hover-row height="600" border="full" keep-source
								@checkbox-change="UpdateInvcode" :auto-resize="$store.state.autoResize"
								:checkbox-config="{ checkField: 'checked', trigger: 'row', range: true, highlight: true }">
								<vxe-column type="checkbox" width="60" align="center"></vxe-column>
								<vxe-column field="invName" width="480" title="物料名称" align="left"></vxe-column>
								<vxe-column field="invName" title="人员分配" align="left">
									<template #default="{ row, rowIndex }">
										<el-tag v-for="tag in row.userList" :key="tag.id" closable size='mini'
											@close="handleClose(tag, rowIndex)"> {{ tag.staffName + '(' + tag.staffNo +
													')'
											}} </el-tag>
									</template>
								</vxe-column>
					...
					<el-row class="subMitBtn" justify="end">
						<vxe-button @click="is_show = false">取消</vxe-button>
						<vxe-button status="primary" @click="Dispatch">保存</vxe-button>
					</el-row>
				</el-main>
			</el-container>
		</el-dialog>
	...
	methods: {
		...
		assigningJob(row) {
			...
			data.forEach((item, index) => {
				let invcodeindex = this.data.inventoryList.findIndex((v) => {
					return v.invCode == item.invCode;
				});
				if (this.data.inventoryList[invcodeindex].hasOwnProperty("user")) {
					if (this.data.inventoryList[invcodeindex].userList.findIndex((v) => { return v.staffNo == row.staffNo }) > -1) {
						return false;
					}
