	...
	<el-container>
		...
		<el-main>
			<vxe-table :data="tableData" 
				 ...
				 resizable
				 show-overflow
				 :expand-config="{accordion:true,visibleMethod:expand_visibleMethod}"
			...
			<vxe-column type="expand" width="40">
			  <template #content="{ row }">
			   ...
			   <el-footer>
			   	   <page ref="c_page_data" :totalCount="children_ch_totalCount"  :pageSize="5"  @page_list="loadContentMethod(row)" />
			   </el-footer>
			   </el-container>
			  </template>
			</vxe-column>
				...
				<vxe-table-column field="capacityMinute10" title="10分钟产能(标准)" width="160px" :edit-render="{name: 'input',enabled:T.CheckPermissions('setReport'),attrs: {type: 'number'}}" ></vxe-table-column>
				<vxe-table-column field="capacityHour"  title="一小时产能(标准)" align="center"  :edit-render="{name: 'input',enabled:T.CheckPermissions('setReport'), attrs: {type: 'number'}}"  width="150px"></vxe-table-column>
				<vxe-table-column field="capacityHour105"  title="10.5产能(标准)" align="center"  :edit-render="{name: 'input',enabled:T.CheckPermissions('setReport'), attrs: {type: 'number'}}"  width="150px"></vxe-table-column>
	...
	export default {
		...
		methods: {
			...
			editClosedEvent(data){
				this.$confirm("确定要将["+data.row.invName+"]修改为自定义产能吗?", "提示", {
					confirmButtonText: "确定",
					cancelButtonText: "取消",
					type: "warning"
				}).then(() => {
			...
			expand_visibleMethod(){
				let visibleMethod=this.T.CheckPermissions('setReport');
