  ...
  <el-container>
    <el-header>
      <vxe-toolbar ref="candidateToolbar" custom print export>
        <template #buttons>
          ...
          <vxe-button v-has="['out_blacklist']" @click="moveFromBlacklist">移出黑名单</vxe-button>
          <vxe-button v-has="['view']" @click="view">查看</vxe-button>
    ...
    <el-main>
      <vxe-table :data="tableData" highlight-current-row highlight-hover-row height="auto" border="full" ref="DataList"
        ...
        <vxe-table-column field="departmentName" title="应聘部门" width="120px" align="center"></vxe-table-column>
        <vxe-table-column field="postName" title="应聘岗位" width="150px" align="center"></vxe-table-column>
        <vxe-table-column field="recruiterName" title="招聘人员" width="80px" align="center"></vxe-table-column>
        ...
        <vxe-table-column field="isHire" title="是否录用" width="90px" align="center">
          <template #default="{ row }">
            <font color="red" v-if="row.isHire == '是'">{{ row.isHire }}</font>
            <font color="orange" v-else-if="row.isHire == '待确认'">{{ row.isHire }}</font>
            <font color="" v-else>{{ row.isHire }}</font>
          </template>
        </vxe-table-column>
        ...
        <vxe-table-column field="createTime" title="录入时间" width="200px" align="center"></vxe-table-column>
      </vxe-table>
    </el-main>
    <el-footer>
    ...
    <Dialog width="600px" title="查询" ref="search" @page_list="page_list">
      <el-form label-width="120px">
        <el-form-item label="招聘人员">
          <el-select v-model="search.recruiterId" placeholder="请选择招聘人员" class="search-row" size="medium">
            <el-option v-for="(item, key) in recruiterList" :key="key" :label="item" :value="key"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="部门">
          <el-select v-model="search.departmentId" placeholder="请选择部门" class="search-row" size="medium">
        ...
        <el-form-item label="是否复试">
          <el-select v-model="search.isArrive" placeholder="请选择" class="search-row" size="medium">
            <el-option value="" label="全部"></el-option>
            <el-option v-for="(item, key) in judgementList" :key="key" :label="item" :value="key"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="是否录用">
          <el-select v-model="search.isHire" placeholder="请选择" class="search-row" size="medium">
            <el-option value="" label="全部"></el-option>
            <el-option v-for="(item, key) in judgementList" :key="key" :label="item" :value="key"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="录入日期">
          <el-date-picker v-model="search.createDate" type="daterange" range-separator="至" start-placeholder="开始日期"
            end-placeholder="结束日期" size="medium" format="YYYY-MM-DD" value-format="YYYY-MM-DD" />
        </el-form-item>
      </el-form>
    </Dialog>
    <save ref="save" @page_list="page_list" />
    <view_info ref="view_info" @page_list="page_list" />
...
import view_info from "@/views/hr/candidate/view.vue";
...
export default {
  ...
  methods: {
    ...
    page_list: function () {
      ...
      this.api.hr.Candidate.GetList(pagesize, page, this.search).then(
        (res) => {
          ...
          this.judgementList = res.data.data.judgmentOption;
        }
      );
    },
    ...
    save: function (isEdit) {
      ...
      this.api.hr.Candidate.EditInfo(id).then((res) => {
        if (res.data.code == 200) {
          ...
          // }
          row.contactList = res.data.data.contactList;
          for (var i in row.contactList) {
            row.contactList[i].checked = false;
          }
          row.postId = (row.postId || "").toString();
          this.$refs.save.saveForm = row;
          this.$refs.save.postList = res.data.data.postList;
          ...
          this.$refs.save.contactTypeOption = res.data.data.contactTypeOption
        }
      });
    },
    del: function () {
    ...
    moveFromBlacklist: function () {
      ...
      this.$confirm("确定要移出黑名单吗?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then(() => {
        this.api.hr.Candidate.MoveFromBlacklist(ids).then((res) => {
          if (res.data.code == 200) {
    ...
    view: function () {
      ...
      let id = data[0].id;
      this.api.hr.Candidate.ViewInfo(id).then((res) => {
        if (res.data.code == 200) {
          var row = res.data.data.row;
          row.isArrive = row.isArrive.toString();
          row.isHire = row.isHire.toString();
          ...
          this.$refs.view_info.is_edit = id ? true : false;
          this.$refs.view_info.is_show = !this.$refs.view_info.is_show;
          ...
          row.postId = (row.postId || "").toString();
          this.$refs.view_info.saveForm = row;
          this.$refs.view_info.postList = res.data.data.postList;
          this.$refs.view_info.recruiterList = res.data.data.recruiterList;
          this.$refs.view_info.departmentList = res.data.data.departmentList;
          this.$refs.view_info.postOptions = res.data.data.postOptions;
          this.$refs.view_info.judgmentOption = res.data.data.judgmentOption;
          this.$refs.view_info.priorityOption = res.data.data.priorityOption;
          this.$refs.view_info.channelOption = res.data.data.channelOption;
          this.$refs.view_info.contactTypeOption = res.data.data.contactTypeOption
        }
      });
    },
    contactLog: function () {
