		...
		<el-header>
			<vxe-toolbar ref="AssignToolbar" zoom custom print export>
				<template #buttons>
					...
					<vxe-button @click="Clear" v-has="['DelDispatching']">批量删除派工</vxe-button>
				</template>
			</vxe-toolbar>
		</el-header>
		<el-main>
			<vxe-table :data="tableData" highlight-current-row highlight-hover-row height="auto" border="full"
				show-overflow ref="Assign" :print-config="{}" :filter-config="{ 'remote': remote }"
				@filter-change="filterNameMethod" :auto-resize="$store.state.autoResize"
				...
				<vxe-column field="productName" title="成品编码" width="130px"></vxe-column>
				<vxe-column field="pInvCode" title="母件编码" width="130px"></vxe-column>
				<vxe-column field="pInvName" title="母件名称" width="180px" :filters="filters.pInvName"></vxe-column>
				...
				<vxe-column field="processName" title="工序名称" fixed="right" width="140px" :filters="filters.processName">
				</vxe-column>
				...
				<vxe-column field="assignStatus" title="状态" width="110px" fixed="right" align="center">
					<template #default="{ row }">
						<el-button size='mini' v-if="row.assignStatus == 0" @click="UserDispatchingShow(row)">未派工
						</el-button>
						<el-button type="danger" size='mini' v-if="row.assignStatus == 1"
							@click="UserDispatchingShow(row)">部分派工</el-button>
						<el-button type="success" size='mini' v-if="row.assignStatus == 2"
							@click="UserDispatchingShow(row)">全部派工</el-button>
					</template>
				</vxe-column>
			</vxe-table>
		</el-main>
		...
		<Dialog width="600px" title="查询" ref="search" @page_list="page_list">
			<el-form label-width="120px">
				...
				<el-form-item label="物料编码">
					<el-input v-model.trim="search.invCode" placeholder="请输入" size="small" style="width:350px">
					</el-input>
				</el-form-item>
				<el-form-item label="物料名称">
					<el-input v-model.trim="search.invName" placeholder="请输入" size="small" style="width:350px">
					</el-input>
				</el-form-item>
			</el-form>
		</Dialog>
	...
	methods: {
		...
		DispatchingShow: function () {
			...
			this.api.production.Assign.GetInfoBatch(ids).then(res => {
				if (res.data.code == 200) {
					this.$refs.dispatching.form = res.data.data.dataList;
