      ...
      >
        <el-scrollbar height="450px" style="padding-right:20px">
          <el-form-item label="沟通时间" prop="contactTime">
            <vxe-input v-model="logSaveForm.contactTime" type="datetime" label-format="yyyy-MM-dd HH:mm" maxlength="16" clearable></vxe-input>
          </el-form-item>
          ...
          <el-form-item label="时长" prop="duration" v-if="logSaveForm.type!=3">
            <el-col :span="8">
          ...
          <el-form-item label="截图凭证" prop="pictures" v-if="logSaveForm.type!=3">
            <el-upload 
              ...
              :show-file-list="false"
					  >
					    <i class="fa fa-2x" :class="uploadIcon" style="color: #42A2FA;margin-top: 35px;"></i>
					  </el-upload>
        ...
        <el-form-item style="padding-top: 10px;">
          <el-button type="primary" @click="submitForm('logSaveForm')">保存</el-button>
          <el-button @click="is_show = false;this.$refs['logSaveForm'].resetFields();">取消</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  </div>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        duration: [{ required: true, type: 'number', message: "请输入时长", trigger: "blur",transform:(value)=>Number(value) }],
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          var func = null;
          if(this.logSaveForm.id) {
            func = this.api.hr.Candidate.EditContact;
          } else {
            this.logSaveForm.candidateId = this.candidateId;
            func = this.api.hr.Candidate.AddContact;
          }
          func(this.logSaveForm).then((res) => {
