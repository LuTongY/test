<template>
  <el-container>
    <el-header>
      <vxe-toolbar>
        <template #buttons>
          <vxe-input v-model.trim="search.name" placeholder="部门名称"></vxe-input>
    ...
    <el-main>
      <vxe-table :data="tableData" highlight-current-row highlight-hover-row height="auto" border="full" ref="Department"
        ...
        <vxe-table-column field="name" title="部门名称" width="200px"></vxe-table-column>
        <vxe-table-column field="createUser" title="创建人" width="100px"></vxe-table-column>
        <vxe-table-column field="createTime" title="创建日期" width="160px"></vxe-table-column>
        <vxe-table-column field="updateUser" title="维护人" width="100px"></vxe-table-column>
        <vxe-table-column field="updateTime" title="维护日期" width="160px"></vxe-table-column>
      </vxe-table>
    </el-main>
    <el-footer>
...
export default {
  ...
  methods: {
    page_list: function () {
      ...
      this.api.hr.Department.GetList(
        pagesize,
        page,
        this.search
      ).then(res => {
    ...
    edit: function () {
      let data = this.$refs.Department.getCheckboxRecords();
      if (data.length < 1) { this.$message.error("请先选择部门"); return false } else if (data.length > 1) { this.$message.error("修改只能单选"); return false }
      this.api.hr.Department.EditInfo(data[0].id).then((res) => {
    ...
    del: function () {
      let data = this.$refs.Department.getCheckboxRecords();
      if (data.length < 1) { this.$message.error("请先选择部门"); return false }
      let ids = [];
      ...
      this.$confirm("此操作将删除该部门, 是否继续?", "提示", {
        ...
        this.api.hr.Department.del(ids).then(res => {
          this.page_list();
