  ...
  <el-container>
      ...
      <vxe-toolbar ref="AssignGroupToolbar" custom print export >  
        <template #buttons>
			<el-row>
				...
				<title-date  ref="TitleDate"   @page_list="page_list" v-model:Pdata="search.date" type='month' days='month' format='YYYY-MM'/>
			</el-row>
        </template>
      </vxe-toolbar>
    </el-header>
    <el-main>
      ...
      >
		...
		<vxe-column field="produceDate" title="开工日期"  width="120px" fixed="right"></vxe-column>
		<vxe-column field="workcenterName" title="指派工作中心"  width="120px" fixed="right"></vxe-column>
		<vxe-column field="assignProduceDate" title="指派生产日期"  width="120px" fixed="right"></vxe-column>
  ...
  methods: {
    ...
    del: function() {
     ...
     if (data.length < 1) {this.$message.error("至少选择一个订单");return false}
      ...
      this.$confirm("此操作将删除已指派的工作中心及已指派的人员记录, 是否继续?", "提示", {
