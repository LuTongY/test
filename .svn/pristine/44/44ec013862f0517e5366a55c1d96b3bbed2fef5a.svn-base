<template>
	<div v-dialogdrag class="warpper">
		...
		<el-dialog title="生产计划" v-model="is_show" width="1200px" @open="open" ref="dialog">
			<el-container>
				<el-header>
							...
							<vxe-table 
								...
								:print-config="{}"
			  					:auto-resize="$store.state.autoResize" 
			  					 show-overflow="title">
			  					<vxe-column field="cInvCName" title="系列" width="120px"></vxe-column>
			  					<vxe-column field="csocode" title="销售订单">
								 <template #default='{ row }'>
									 <span>{{row.csocode+'-'+row.iorderseq}}</span>
								 </template>
								</vxe-column>
			  					<vxe-column field="cInvCode" title="成品编码"></vxe-column>
			  					<vxe-column field="cinvName" title="成品名称"></vxe-column>
			  					<vxe-column field="iQuantity" title="入库数量" :formatter="({ cellValue })=>{ return  T.floatRow(cellValue)}" width="80px"></vxe-column>
			  					<vxe-column field="dDate" title="入库日期"  :formatter="({ cellValue })=>{ return   moment(cellValue).format('YYYY-MM-DD')}" width="120px"></vxe-column>
