<template>
	<el-container>
		<el-header>
			<vxe-toolbar ref="purchaseOutsourceToolbar" custom print>
				<template #buttons>
...
<script>
	...
	export default {
		...
		created() {
			this.$nextTick(() => {
				this.$refs.purchaseOutsource.connect(this.$refs.purchaseOutsourceToolbar)
			})
		},
		mounted() {
			this.page_list();
		...
		methods: {
			...
			page_list: function() {
				...
				this.api.purchase.purchaseOutsource.GetList(
					pagesize,
					page,
			...
			Export:function(){
				this.api.purchase.purchaseOutsource.Export(this.$refs.search.form).then((res)=>{
					if (res.data.code == 200) {
