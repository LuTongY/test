	...
	<el-form :model="roleForm" :rules="rules" ref="roleForm" label-width="100px" class="demo-ruleForm">
			 <el-form-item label="工序名称" prop="name" >
			   ...
			   <el-form-item  label="所属工艺">
			       <el-select v-model="roleForm.craftId"  placeholder="请选择工艺" size="small">
			   						<el-option v-for="(item,key) in options.crafts" :label="item" :value="key" :key="key">
			   						</el-option>
			   					</el-select>
			   </el-form-item>
			...
			submitForm(formName) {
                console.log(this.roleForm)
			        this.$refs[formName].validate((valid) => {
			          if (valid) {
			             this.api.production.Process.add(this.roleForm).then((res)=>{
...
#add /deep/ .el-input__suffix-inner i, .el-input__prefix i{
    height: 32px;
}
