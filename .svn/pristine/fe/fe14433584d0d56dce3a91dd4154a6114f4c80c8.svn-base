<template>
  <el-container>
    <el-header>
      <vxe-toolbar ref="PackMaterialCardToolbar" custom print>
        <template #buttons>
          ...
          <vxe-button v-has="['print']" @click="open">打印物料卡</vxe-button>
        </template>
      </vxe-toolbar>
    </el-header>
    <el-main>
	...
	<MyDialog ref='print' title='打印标识卡' width='450px' @DialogBtn='DialogBtn'>
	   <el-form  :rules="rules"  ref="MyDialog_form"  :model="MyDialogForm">
		   <el-form-item label='每个展板数量' prop="number">
			  <vxe-input v-model="MyDialogForm.number" placeholder="请输入数字"  min="0" type="number"></vxe-input>
		   </el-form-item>
		   <el-form-item label='是否将尾数归入最后一板'>
		   		<el-checkbox v-model="MyDialogForm.is_include" name="type"></el-checkbox>
		   </el-form-item>
	   </el-form>
	</MyDialog>
   <print ref="DialogPrint" width='800px' @page_list='page_list' />
  </el-container>
</template>
<script>
...
created () {
  ...
  methods: {
	...
	open:function(){
		 let row=this.$refs.PackMaterialCard.getRadioRecord()
		 if (!row) {this.$message.error("请先选择订单");return false}
		 this.api.warehouse.PackMaterialCard.ViewInfo(row.id).then((res)=>{
			 
		 })
          this.$refs.print.isShow=true;
	},
	DialogBtn:function(){
		let row=this.$refs.PackMaterialCard.getRadioRecord();
		 row.MyDialogForm=this.MyDialogForm
		  this.$refs.DialogPrint.rowData=row;
		  this.$nextTick(()=>{
			  this.$refs.DialogPrint.Panels();
		  })
		  this.$refs.MyDialog_form.validate(valid => {
		    if (valid) {
				 this.$refs.print.isShow=false;
				 let data={
					 quantity:this.MyDialogForm.number,
					 includeRemain:this.MyDialogForm.is_include,
				 }
				this.api.warehouse.PackMaterialCard.View(data).then((res)=>{
					if(res.data.code==200){
						this.$refs.DialogPrint.isShow()
					}
				})           
		    } else {
		      return false;
