	...
	methods: {
		checkQuantity(e, e2) {
			...
			for (let i in this.originData) {
				if (this.originData[i].id == e.row.id && parseInt(e2.target.value) > parseInt(this.originData[i].quantity)) {
					this.$message.error("本次加工量不能大于可加工量");
					setTimeout(function () {
						that.tableData[i].quantity = that.originData[i].quantity;
						if (that.selectedIds[e.row.id]) {
							that.selectedIds[e.row.id].quantity = that.originData[i].quantity;
						}
					}, 500)
					return false;
				}
		...
		page_list(resetPage) {
			...
			this.api.production.Task.GetListWithProcessChoose(pagesize, page, this.search).then(res => {
				if (res.data.code == 200) {
					...
					for (let i in rows) {
						this.originData.push({
							...
							quantity: res.data.data.rows[i].quantity,
						});
						if (this.selectedIds[rows[i].id] && this.selectedIds[rows[i].id].checked) {
							rows[i].checked = true;
						} else {
